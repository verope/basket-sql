...
create
or replace temp table "itesco_most_recent_itemName" as
select "itemId",
       "itemName",
       "currentPrice",
       "itemUrl",
...
select distinct *,
                case
                    when "itesco_nazev" = ''KAKAOVÝ PRÁŠEK'' and "productId" = ''2005100242357 '' and "poradi" = 1 then ''stringSearch''
...
create
or replace table "search_bot_itesco_to_clean" as
select *
...
create
or replace table "itesco_id_to_delete_u" as
select distinct trim(id) as "id"
from "itesco_id_to_delete";

-- kategorie s malym poctem produktu -> manualni search
create
temp table "itesco_okurky_doplneni" as
select "itemId"::string || ''_E01_173_02'' as "productId_csuId_ident", '' STERILOVANÉ OKURKY''              as "csu_nazev", null as "itesco_notes",
       "itemName",
       "itemUrl",
       ''      NAKLÁDANÉ OKURKY''                as "itesco_nazev", "itemId" as "productId",
       null as "poradi",
       ''      manualSearch''                    as "searchType", '' E01_173_02 ''                      as "csu_id"
from (select *, row_number() over (partition by "itemId" order by "date" desc) as rown
      ...
      where "itemId" in
            (2001020257049,
             ...
             2001120367028,
             2001019076381,
             2001120351675) qualify rown = 1);

create
temp table "itesco_kompot_doplneni" as
select "itemId"::string || ''_E01_164_01''     as "productId_csuId_ident", '' KOMPOT MERUŇKOVÝ (EVENT. BROSKVOVÝ)'' as "csu_nazev", null as "itesco_notes",
       "itemName",
       "itemUrl",
       ''      BROSKVE V NÁLEVU''                    as "itesco_nazev", "itemId" as "productId",
       null as "poradi",
       ''      manualSearch''                        as "searchType", '' E01_164_01 ''                          as "csu_id"
from (select *, row_number() over (partition by "itemId" order by "date" desc) as rown
      from "itesco_clean"
      where "itemId" in
            (2001019074868,
             ...
             2001018528133,
             2001019074844,
             2001017545049) qualify rown = 1);

create
temp table "itesco_margarin_doplneni" as
select "itemId"::string || ''_E01_152_01'' as "productId_csuId_ident", '' ROSTLINNÝ ROZTÍRATELNÝ TUK''      as "csu_nazev", null as "itesco_notes",
       "itemName",
       "itemUrl",
       ''      MARGARÍN''                        as "itesco_nazev", "itemId" as "productId",
       null as "poradi",
       ''      manualSearch''                    as "searchType", '' E01_152_01 ''                      as "csu_id"
from (select *, row_number() over (partition by "itemId" order by "date" desc) as rown
      from "itesco_clean"
      where "itemId" in
            (
...
select "productId_csuId_ident",
       ...
       "itemUrl",
       "itesco_nazev",
       "productId"::int as "productId", "poradi",
       "searchType",
       "csu_id"
...
create
or replace temp table "itesco_kos_ean_itemId_array" as
select ean,
       array_agg("itemId") as "itemId_array",
       case
           when "itemId_array"[0]::int < "itemId_array"[1]::int then "itemId_array"[0]::int
           else "itemId_array"[1]::int
end as "itemId_1", -- mensi id prvni
       case
           when "itemId_array"[0]::int > "itemId_array"[1]::int then "itemId_array"[0]::int
           else "itemId_array"[1]::int
end as "itemId_2"  -- vetsi id druhy
...
create
or replace table "search_bot_itesco_clean_vero_ean_fix" as
select t1.*,
       case
...
create
or replace table "itesco_spotrebni_kos_vyvoj_ean_fix" as
...
CREATE
OR REPLACE TABLE "itesco_tmp1" AS
WITH "pom" AS (SELECT *,
                      CASE
                          WHEN "itemId_given_day" = ''
2001120571270'' AND TO_NUMBER("basePrice") > ''
70''
...
/* tu sa zasa riesia jednodnove prepady, ked zabudli prehodit zlacnenu cenu, ale kedze je Tesco ne-idealni, nakoniec kazdy den po skonceni zlavy beriem predchadzajucu cenu */
CREATE
OR REPLACE TABLE "itesco_spotrebni_kos_vyvoj_ppu_pom" AS
SELECT "productId_csuId_ident",
       "csu_nazev",
       "found_product_ids",
       "itemName",
       ...
       LAG("originalPrice") OVER (PARTITION BY "itemId_given_day" ORDER BY "date") <> '''' THEN LAG("ppu_final") OVER (PARTITION BY "itemId_given_day" ORDER BY "date")
           ELSE "ppu_final"
END
AS "price_per_unit",
       "unit"
...
CREATE
OR REPLACE TABLE "itesco_spotrebni_kos_vyvoj_ppu_pom2" AS
SELECT *
...
FROM "itesco_spotrebni_kos_vyvoj_ppu_pom" QUALIFY "old_item_fix_rn" = 1;

create
or replace table "itesco_spotrebni_kos_vyvoj_ppu" as
select INITCAP(t2.nazev_hlavni_kategorie, '''') as                       "csu_main_category",
       ...
       t1."found_product_ids"::string         as "it_product_id_array", t1."itemName" as "itemName",
       t1."nove_poradi",
       t1."p_key"                               as                       "p_key",
       t1."itemId_given_day",
       ...
       t1."basePrice",
       CASE
           WHEN "csu_product" = ''Česnek suchý'' AND "package_unit" = '' ks '' THEN 20 * "price_per_unit" * "csu_amount" -- jeden cesnek vazi cca 0.05kg = tomu odpovida price per unit, tzn krat 20 je to na kilo
...
/*
"csu_product"
''Kiwi'' -- musi mit velikost nebo to nejde - jen jeden produkt
...
CREATE
OR REPLACE TABLE "itesco_spotrebni_kos_product_selection" AS
SELECT *
     , LAG("avg_csuRelevantPrice")
    OVER
    (PARTITION BY "csu_product" ORDER BY "avg_csuRelevantPrice") AS "avg_price_diff_from_prev"
FROM (SELECT *
      ...
      FROM (SELECT "csu_product"
                 , COUNT(DISTINCT "itemName", "it_product_id_array")
                                                                      OVER
                (PARTITION BY "csu_product")                                     AS "items_in_category"
                 , "itemName"
                 , "it_product_id_array"
                 , "country"
                 , "nove_poradi"
                 ...
                 , "package_unit"
                 , CASE
                       WHEN "csu_product" = ''Droždí'' AND "itemName" ILIKE
                ''%sušené%'' THEN 1
...
CREATE
OR REPLACE TABLE "itesco_spotrebni_kos_product_selection_fin" AS
SELECT "csu_product"
     , "items_in_category"
     , "itemName"
     , "it_product_id_array"
...
CREATE
TEMP TABLE KOSTRA AS
SELECT DISTINCT "date"
FROM "itesco_spotrebni_kos_vyvoj_ppu" T1
ORDER BY 1;

CREATE
OR REPLACE TEMP TABLE KOSTRA_2 AS
...
CREATE
OR REPLACE TEMP TABLE KOSTRA_3 AS
SELECT *
FROM KOSTRA_2 T1
...
CREATE
OR REPLACE TEMP TABLE "first_known_price" AS
SELECT T1.*
     , "csu_main_category"
     , "csu_subcategory"
     , "csu_product"
...
CREATE
OR REPLACE TEMP TABLE KOSTRA_4 AS
...
CREATE
OR REPLACE TEMP TABLE "dopocitani_non_null" AS
...
CREATE
OR REPLACE TEMP TABLE "dopocitani_non_null2" AS
SELECT *
     , LEAD("kostra_date_nn", 1, CURRENT_DATE())
    OVER
    (PARTITION BY "kostra_itemName_nn", "kostra_it_product_id_array_nn" ORDER BY "kostra_date_nn") AS "non_null_end"
...
CREATE
OR REPLACE TEMP TABLE "dopocitani_non_null_fin" AS
...
CREATE
OR REPLACE TABLE "out_itesco_spotrebni_kos_full_hist" AS
SELECT T1.*
...
CREATE
OR REPLACE TABLE "itesco_spotrebni_kos_vazena_suma" AS
...
CREATE
OR REPLACE TABLE "out_itesco_spotrebni_kos_main_cat_agg" AS
SELECT "date"
     , "csu_main_category"
...
CREATE
OR REPLACE TABLE "out_itesco_spotrebni_kos_sub_cat_agg" AS
SELECT "date"
     , "csu_main_category"
     , "csu_subcategory"
     , AVG("filled_csuRelevantPrice") AS "csuRelevantPrice"
...
CREATE
OR REPLACE TABLE "out_itesco_spotrebni_kos_product_agg" AS
SELECT "date"
     , "csu_main_category"
     , "csu_subcategory"
     , "csu_product"
...
CREATE
OR REPLACE TABLE "out_itesco_spotrebni_kos_item_agg" AS
SELECT "date"
     , "csu_main_category"
     , "csu_subcategory"
     , "csu_product"
